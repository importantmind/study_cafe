...
public class SeatView2 {
	...
	//로그인 컨트롤러의 현재회원 가져와서 넣으면됨
	
	//1-1. 좌석 중복 배제
	public void seatView() {
		//num은 초기값 0
		if(lc.getMember().getNum() != 0) { //로그인컨트롤러의 members 리스트의 '현재 사용자(get(?)' 의 번호가 0이 아니면 선택불가능
	...
	//1-2. 좌석을 3자리씩 3줄로 프린트
		...
		for(int i = 6; i < 9; i++) {
			...
			if(false == sct.getSeat().get(num-1).isSet()) {
				...
				lc.getMember().setNum(num); //로그인컨트롤러의 members 리스트의 '현재 사용자(get(?)' 의 번호를 num으로 바꾼다. 
				go = false;
				break;
			}else{
				//자리 선택불가
	...
	public void seatMove() {
		...
		for(int i = 6; i < 9; i++) {
			...
			if(false == sct.getSeat().get(num-1).isSet()) {
				...
				lc.getMember().setNum(num2); //로그인컨트롤러의 members 리스트의 '현재 사용자(get(?)' 의 번호를 num2로 바꾼다. 
				go = false;
				break;
			}else {
				//자리 선택불가
	...
	public void seatReturn() {
		...
		lc.getMember().setNum(0); //로그인컨트롤러의 members 리스트의 '현재 사용자(get(?)' 의 번호를 0으로 바꾼다. 
		break;
			}
		}
	}
